...
Generate attendance consultation forms from an attendance export. Argument: $ARGUMENTS

## Steps

1. Parse the argument:
   - If a `.xlsx` file path is given: use that file directly.
   - If no argument given: search the agent folder for any `.xlsx` file. If found, confirm with the instructor. If not found, ask them to drop the attendance export into the agent folder.

2. Determine flags:
   - If the instructor says "all students" or "include zero absences": add `--all` flag.
...
4. After running:
   - Print the counseling summary from the terminal output so the instructor can see who needs follow-up.
...
## Do NOT ask for:
- Course name, instructor name, semester, or any other info. All of that is read automatically from the Excel file.
